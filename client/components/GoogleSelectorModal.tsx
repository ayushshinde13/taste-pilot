'use client'

import { useState } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { X, ArrowLeft, Loader2 } from 'lucide-react'
import { useAuth } from '@/context/AuthContext'
import { useRouter } from 'next/navigation'

interface GoogleSelectorModalProps {
  isOpen: boolean
  onClose: () => void
  redirectPath?: string
}

const mockAccounts = [
  {
    name: 'Ayush Developer',
    email: 'ayush.developer@gmail.com',
    avatar: '/avatars/male_1.png',
  },
  {
    name: 'Guest User',
    email: 'guest.user@gmail.com',
    avatar: '/avatars/male_2.png',
  },
  {
    name: 'Test Account',
    email: 'test.tastepilot@gmail.com',
    avatar: '/avatars/female_1.png',
  },
]

export default function GoogleSelectorModal({ isOpen, onClose, redirectPath = '/restaurants' }: GoogleSelectorModalProps) {
  const { googleLogin } = useAuth()
  const router = useRouter()
  
  const [view, setView] = useState<'chooser' | 'custom'>('chooser')
  const [customEmail, setCustomEmail] = useState('')
  const [customPassword, setCustomPassword] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  if (!isOpen) return null

  const handleSelectAccount = async (account: { email: string; name: string; avatar: string }) => {
    setIsLoading(true)
    setError(null)
    try {
      await googleLogin(account.email, account.name, account.avatar)
      onClose()
      router.push(redirectPath)
      router.refresh()
    } catch (err: any) {
      setError(err.message || 'Failed to sign in with Google')
      setIsLoading(false)
    }
  }

  const handleCustomSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!customEmail.endsWith('@gmail.com') && !customEmail.includes('@')) {
      setError('Please enter a valid email address')
      return
    }
    if (!customPassword) {
      setError('Password is required')
      return
    }
    if (customPassword.length < 6) {
      setError('Password must be at least 6 characters')
      return
    }
    
    setIsLoading(true)
    setError(null)
    try {
      const defaultAvatar = '/avatars/male_3.png'
      const generatedName = customEmail.split('@')[0]
      await googleLogin(customEmail.trim(), generatedName, defaultAvatar)
      onClose()
      router.push(redirectPath)
      router.refresh()
    } catch (err: any) {
      setError(err.message || 'Failed to sign in with Google')
      setIsLoading(false)
    }
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      {/* Modal Container */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 15 }}
        transition={{ duration: 0.2 }}
        className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white p-6 shadow-2xl border border-gray-100 flex flex-col justify-between min-h-[420px]"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          disabled={isLoading}
          className="absolute right-4 top-4 rounded-full p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition disabled:opacity-50"
        >
          <X size={20} />
        </button>

        {/* Google Header */}
        <div className="flex flex-col items-center text-center mt-2 mb-4">
          <svg className="h-8 w-8 mb-3" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          <h2 className="text-xl font-bold text-gray-950">Sign in with Google</h2>
          <p className="text-sm text-gray-500 mt-1">to continue to Taste Pilot</p>
        </div>

        {error && (
          <div className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-600 font-medium">
            {error}
          </div>
        )}

        {/* Content body */}
        <div className="flex-1 flex flex-col justify-center">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-8">
              <Loader2 className="animate-spin text-orange-500 h-10 w-10 mb-4" />
              <p className="text-sm font-semibold text-gray-700">Connecting to Google Auth...</p>
            </div>
          ) : view === 'chooser' ? (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                Choose an account
              </p>
              
              {/* Account Options List */}
              <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden bg-white shadow-sm">
                {mockAccounts.map((account) => (
                  <button
                    key={account.email}
                    onClick={() => handleSelectAccount(account)}
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50/80 transition text-left active:bg-gray-100"
                  >
                    <div className="h-9 w-9 rounded-full overflow-hidden border border-gray-200 bg-gray-50">
                      <img src={account.avatar} alt={account.name} className="h-full w-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-gray-900 truncate">{account.name}</p>
                      <p className="text-xs text-gray-500 truncate">{account.email}</p>
                    </div>
                  </button>
                ))}
              </div>

              {/* Use Another Account Button */}
              <button
                onClick={() => setView('custom')}
                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition text-left rounded-xl border border-dashed border-gray-300 mt-2 hover:border-gray-400 text-gray-600 hover:text-gray-800"
              >
                <div className="h-9 w-9 rounded-full border border-gray-200 bg-gray-50 flex items-center justify-center text-lg text-gray-500">
                  👤
                </div>
                <span className="text-sm font-semibold">Use another account</span>
              </button>
            </div>
          ) : (
            /* Custom Email Input View */
            <form onSubmit={handleCustomSubmit} className="space-y-4">
              <div className="flex items-center gap-2 mb-2 text-gray-600">
                <button
                  type="button"
                  onClick={() => setView('chooser')}
                  className="p-1 hover:bg-gray-100 rounded-full transition"
                >
                  <ArrowLeft size={18} />
                </button>
                <span className="text-sm font-bold">Use a custom account</span>
              </div>

              <div>
                <label htmlFor="customEmail" className="block text-xs font-semibold text-gray-600 mb-1">
                  Gmail Address *
                </label>
                <input
                  type="email"
                  id="customEmail"
                  placeholder="e.g. yourname@gmail.com"
                  value={customEmail}
                  onChange={(e) => setCustomEmail(e.target.value)}
                  required
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition"
                />
              </div>

              <div>
                <label htmlFor="customPassword" className="block text-xs font-semibold text-gray-600 mb-1">
                  Gmail Password *
                </label>
                <input
                  type="password"
                  id="customPassword"
                  placeholder="Enter your Gmail password"
                  value={customPassword}
                  onChange={(e) => setCustomPassword(e.target.value)}
                  required
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition"
                />
              </div>

              <button
                type="submit"
                className="w-full py-2 px-4 rounded-lg text-white font-bold text-sm bg-orange-600 hover:bg-orange-700 shadow-sm transition hover:shadow active:scale-[0.99]"
              >
                Sign In & Continue
              </button>
            </form>
          )}
        </div>

        {/* Footer */}
        <div className="text-center text-[10px] text-gray-400 border-t border-gray-100 pt-3 mt-4">
          To continue, Google will share your name, email address, and profile picture with Taste Pilot.
        </div>
      </motion.div>
    </div>
  )
}
